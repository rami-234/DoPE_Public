...
n_lsoa=function(x,df=df_all_lsoas_years) {
  model=nrow(df_all_lsoas_years[df_all_lsoas_years$year==x,])
  return(model)}
...
avrg_perc_bme=function(x,df=lsoa_df) {
  model=mean(lsoa_df$perc_bme[lsoa_df$year==x],  na.rm=T)
  return(model)}
...
avrg_imd=function(x,df=lsoa_df) {
  model=mean(lsoa_df$imd[lsoa_df$year==x],  na.rm=T)
  return(model)} 
