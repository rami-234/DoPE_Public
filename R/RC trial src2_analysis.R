...
# LOAD DATA
#===
#RC changes in code below: csv file; not dividing by weeks, one day only
df <- read.csv("./output/lsoa_df_05Aug2017.csv") %>%
  dplyr::select(run_count,imd, perc_bme, mn_dstn,  
                total_pop, pop_density,perc_non_working_age) %>% 
  mutate(run_rate = run_count/total_pop * 1000)

#===
# DESCRIPTIVE STATISTICS - TABLE 2
#===
#RC note: I need to feed a dataset to stargazer
...
#===
#RC change below: csv file name
cor_mat <-  read.csv("./output/lsoa_df_05Aug2017.csv") %>%
  dplyr::select(run_count,imd, perc_bme, mn_dstn,  
                total_pop, pop_density,perc_non_working_age) %>%
  cor
...
ggsave(filename = "./output/colour_plot_05Aug2017.png",plot = plot1,device = "png")
